function dist = dist_Linfbetweeen(x, y)
% dist = dist_Linfbetweeen(x, y)
% Compute the sum of pairwise L_inf distance between vectorized
% networks x  and y.
%
...
for jj = 1:size(x,1)
    ...
    vec = [vec;vecnorm(diff_jj,Inf,2)]; %if vecnorm(diff_jj,1,2), we get L1-distance
end
dist = sum(vec);
